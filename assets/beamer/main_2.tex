...
Schrodinger (1926) derived the Time Dependent Equation:
Give it a system of a single particle, let $\Psi$ be a complex value function called \textbf{wave function}, $\hat{H}$ the \textbf{Hamiltonian Operator}
...
\end{equation}
$\hat{V}$ is the potential operator (internal, external), $\hat{\vec{P}}$ is the momentum operator and $\nabla$ is the laplacian operator.
\end{block}
\end{frame}
...
An observable $O$ is represented by a Hermitian operator $\hat{O}$.

The expectation value (average outcome of many measurements) is
...
If $\Psi$ is an eigenfunction of $\hat{O}$,

\[
\hat{O}\Psi = o\Psi
\]

then the measurement of $O$ gives the definite value $o$.
...
\begin{figure}
  \begin{center}
    \includegraphics[width=0.25\textwidth]{img/density.png}
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{img/walrus.png}
    \caption{Caption}
    \label{fig:placeholder}
\end{figure}
...
An internal representation that a system builds of its environment’s dynamics so it can simulate how the world changes in response to actions
\end{block}

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{img/Lingbot.png}
    \caption{Caption}
\end{figure}
...
Multimodal model that sees the world through visual input, understands human instructions in natural language, and produces action outputs that can directly control an agent like a robot.
\end{block}
