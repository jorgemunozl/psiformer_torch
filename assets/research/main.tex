...
In this section I described how the experiments were did it.

\subsection{Software and computational environment}

All experiments were implemented in \texttt{Python} using \texttt{PyTorch} \cite{Paszke2019PyTorch},
selected for its dynamic computation graph, mature automatic differentiation, and broad ecosystem
support for attention-based architectures. The codebase was developed and executed under
\texttt{Python~X.Y} and \texttt{PyTorch~A.B}, with GPU acceleration via \texttt{CUDA~C.D}.
Experiments were run on rented GPU instances, using NVIDIA GPUs
(\textit{model}, \textit{VRAM}) to support the computational cost of VMC sampling, determinant
evaluations, and long optimization horizons typical of neural quantum states.

\subsection{Experiment tracking and artifacts}

Training runs were tracked using Weights \& Biases, which logs hyperparameters, energy estimates,
variance metrics, and run metadata for each experiment. The complete implementation is publicly
available at \texttt{<repository link>}, including scripts to reproduce all reported figures.
Model checkpoints corresponding to the reported results are archived on Hugging Face at
\texttt{<HuggingFace link>}. Each artifact is associated with a repository commit hash and a unique
experiment identifier to ensure traceability from plots to training configuration.

\subsection{Reproducibility protocol}

To support reproducibility, we (i) fix random seeds for model initialization and sampling,
(ii) record full hyperparameter configurations and software versions in the experiment logs,
and (iii) provide deterministic evaluation scripts for computing reported energies from stored
checkpoints. The repository includes a configuration file specifying dependencies and a minimal
command-line interface for launching training and evaluation.


\begin{table}[t]
\centering
\caption{Software environment and dependencies for \texttt{psiformer-torch}.}
\label{tab:env_psiformer_torch}
\begin{tabular}{p{0.34\linewidth} p{0.58\linewidth}}
\hline
\textbf{Category} & \textbf{Specification} \\
\hline
Project name & \texttt{psiformer-torch} \\
Version & \texttt{0.1.0} \\
Language & Python \\
Python requirement & \texttt{$\ge$ 3.11} \\
Dependency manager & \texttt{uv} (via \texttt{pyproject.toml}) \\
Core dependencies & \texttt{numpy ($\ge$2.3.3)}, \texttt{matplotlib ($\ge$3.10.7)}, \texttt{torchviz ($\ge$0.0.3)}, \texttt{wandb ($\ge$0.23.0)} \\
Dependency groups & \texttt{cuda}: \texttt{torch ($\ge$2.9.1)}; \texttt{laptop}: \texttt{torch ($\ge$2.9.1)} \\
Package indexes & \texttt{PyPI} (default); \texttt{pytorch-cu128}: \texttt{https://download.pytorch.org/whl/cu128} (explicit) \\
Torch source resolution & \texttt{torch} forced from \texttt{pytorch-cu128} index (CUDA~12.8 wheels) \\
Experiment tracking & Weights \& Biases (\texttt{wandb}) \\
Compute hardware & NVIDIA GeForce RTX 4080 (GPU) \\
\hline
\end{tabular}
\end{table}
...
After the training we can observe how the energy predicted of the model is goes to 
the energy produced by, this tell us how 

\section{Observation}\label{Observation}

The training of the model could be quite unstable because when computing the Laplacian
of because the second derivative is nearly singular and the det becomes zero, 
we can fix this by using.

If we want to leverage all the computaional power from the GPU,
we need to make some tricks.


\section{Conclusion}\label{Conclusion}

We conclude by saying that Psiformer was able to obtain the ground state energy for the 
atoms Hidrogen Helium 
