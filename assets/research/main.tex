...
\usepackage{braket}
\usepackage{graphicx}
\title{PsiFormer: A Transformer Architecture for Many-Body Quantum Mechanics}
...
\end{equation}
Since in practice is better to work in the log-space because is more numerically stable. We use the follow form of the kinetic term. For a any derivable function \(f\) we know that:
\begin{equation}
  \frac{\nabla^{2}f}{f}=[\nabla^{2}\log f+(\nabla \log f)^{2}]
\end{equation}
Thus we obtain:
...
This section describes the training and evaluation procedure used to obtain the results reported in this work. The overall goal is to approximate the electronic ground-state wave function for atoms using \emph{only} the nuclear charge and the electron coordinates, without supervised labels or external datasets. The PsiFormer model is trained by minimizing the variational energy through Variational Monte Carlo (VMC).

\subsection{Variational Monte Carlo objective}
Let $\psi_\theta(\mathbf{R})$ be the PsiFormer wave function with parameters $\theta$, where
$\mathbf{R} = (\mathbf{r}_1,\dots,\mathbf{r}_N)$ denotes the $3N$-dimensional electron configuration.
We minimize the variational energy
\begin{equation}
E(\theta) \;=\; \frac{\langle \psi_\theta \vert \hat{H} \vert \psi_\theta \rangle}{\langle \psi_\theta \vert \psi_\theta \rangle}
\;=\; \mathbb{E}_{\mathbf{R}\sim |\psi_\theta|^2}\left[E_L(\mathbf{R};\theta)\right],
\end{equation}
where $E_L(\mathbf{R};\theta)=\frac{\hat{H}\psi_\theta(\mathbf{R})}{\psi_\theta(\mathbf{R})}$ is the local energy. In practice, expectations are approximated by Monte Carlo samples drawn from the probability density proportional to $|\psi_\theta(\mathbf{R})|^2$.

\subsection{Sampling procedure}
Electron configurations are drawn using Markov Chain Monte Carlo (MCMC), with multiple parallel chains (“walkers”) to reduce correlation and improve statistical efficiency. At each training iteration, we:
\begin{enumerate}
  \item propagate walkers for a fixed number of MCMC steps,
  \item collect a batch of configurations $\{\mathbf{R}_k\}$,
  \item estimate the energy and its gradient using the sampled local energies.
\end{enumerate}

\subsection{Optimization and training setup}
Parameters are optimized by gradient-based minimization of the variational energy estimator. Training is performed separately for a \textbf{small} and a \textbf{large} PsiFormer configuration, differing in representational capacity (e.g., embedding width, number of layers, number of attention heads, and/or determinant channels). To support reproducibility and interpretability, we report the full training configuration and sampler settings in Table~\ref{tab:hyperparams}.

\subsection{Experiment tracking}
All runs are logged using Weights \& Biases (W\&B), including hyperparameters, energy curves, and evaluation metrics. This facilitates systematic comparison between model sizes and provides a record of training dynamics.

% ---------------------------
% Hyperparameters table (PDF)
% ---------------------------
\begin{table}[t]
\centering
\includegraphics[width=\linewidth]{img/table_hyper.pdf}
\caption{\textbf{Training and sampling hyperparameters used in this work.}
The table summarizes the optimization procedure and the VMC sampling protocol (e.g., walkers, MCMC step size, steps per iteration, equilibration). The \textbf{small} and \textbf{large} PsiFormer models follow the same pipeline, differing only in model capacity settings.}
\label{tab:hyperparams}
\end{table}

\subsection{Evaluation protocol}
After training, ground-state energies are estimated as Monte Carlo averages of the local energy over a held-out sampling phase using the trained model parameters. We report final energy estimates for the first eight atoms considered in this study (Table~\ref{tab:energies}). In addition, we analyze convergence curves for representative atoms (Fig.~\ref{fig:convergence}) and visualize a helium density grid (Fig.~\ref{fig:helium_density}) to probe qualitative structure beyond scalar energies.

% ============================================================
\section{Results and Discussion}
\label{sec:results}

This section presents the quantitative energy results and qualitative diagnostics obtained with PsiFormer. The main comparison is between the \textbf{small} and \textbf{large} configurations trained under the same VMC pipeline.
...
\begin{table}[t]
\centering
\includegraphics[width=\linewidth]{img/table_energy.pdf}
\caption{\textbf{Ground-state energy estimates for the first eight atoms.}
Energies are reported in atomic units (Hartree) as Monte Carlo estimates of the variational energy using the trained wave function. Lower energies indicate better variational bounds.}
\label{tab:energies}
\end{table}

\subsection{Energy estimates for the first eight atoms}
The \cref{tab:energies} summarizes the final ground-state energy estimates obtained for the first eight atoms. Across the set, the \textbf{large} model generally achieves lower energies (i.e., better variational bounds) than the \textbf{small} model, consistent with increased representational capacity. The gap between small and large models tends to become more visible as the number of electrons increases, reflecting the growing difficulty of capturing correlation effects and antisymmetry constraints in heavier atoms.

\subsection{Convergence behavior}
Figure~\ref{fig:convergence} shows training curves for four representative atoms. The energy decreases overall but exhibits stochastic fluctuations due to Monte Carlo noise in the local energy estimator. The large model typically reaches a lower-energy regime than the small model, though it may require more careful optimization to avoid plateaus or high-variance phases early in training.
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{img/convergence_plot.pdf}
\caption{\textbf{Training convergence for four representative atoms.}
Variational energy estimates during training are shown for both the \textbf{small} and \textbf{large} PsiFormer models. Curves reflect stochastic optimization under VMC sampling and therefore include Monte Carlo noise. The large model consistently trends to lower energies, indicating improved variational bounds with increased capacity.}
\label{fig:convergence}
\end{figure}

\subsection{Helium density grid}

To probe the learned wave function beyond scalar energies, Fig.~\ref{fig:helium_density} visualizes a density grid for the $\mathrm{H}_2$ molecule. The density is concentrated around the two nuclei and shows the characteristic build-up of probability in the bonding region between them, providing a qualitative diagnostic that the wave function encodes physically meaningful spatial structure. In this work, the density visualization is included as a reference example (adapted from QMCTorch) to illustrate the type of real-space structure that VMC wave functions represent.

% ---------------------------
% Helium density grid (PDF)
% ---------------------------
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{img/density.pdf}



\caption{\textbf{Real-space density grid for $\mathrm{H}_2$ (reference visualization).}
Electron density for the $\mathrm{H}_2$ molecule showing localization around the two nuclei and enhanced density in the internuclear region consistent with covalent bonding. This figure is taken from QMCTorch and is shown here as a qualitative reference for density-based diagnostics.}
\label{fig:helium_density}
\end{figure}

\subsection{Limitations observed}
The experiments highlight practical limitations: (i) scaling to heavier atoms increases the difficulty of sampling and optimization, (ii) Monte Carlo noise can slow convergence and makes hyperparameters unusually influential.

% ============================================================
\section{Conclusion}
\label{sec:conclusion}

This work presented PsiFormer, a Transformer-based variational wave-function model trained using Variational Monte Carlo without external supervision. We evaluated two capacity regimes (small vs.\ large) and reported ground-state energy estimates for the first eight atoms. The results show that increasing model capacity improves the variational energy, and that PsiFormer learns physically meaningful structure as supported by both convergence curves and a helium density visualization.

At the same time, performance remains sensitive to sampling efficiency and optimization choices, especially as the number of electrons increases. Future work will focus on improving MCMC proposals, reducing estimator variance, and strengthening physics-informed constraints (e.g., cusp behavior and asymptotics), as well as exploring more robust optimization strategies such as preconditioning or natural-gradient-style updates.

% ============================================================
\section*{Acknowledgements}
I thank the Computer Science Faculty for providing access to GPU resources, in particular an NVIDIA GeForce RTX 4080 Super, (``Monstrito'') which enabled the training and evaluation of PsiFormer models reported in this work.
